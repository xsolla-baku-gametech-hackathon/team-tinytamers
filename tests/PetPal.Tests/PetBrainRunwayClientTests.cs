...
public class PetBrainRunwayClientTests
{
    ...
    public async Task Sorgu_VersiyaBasligiDasiyir_AcarSizmir()
    {
        ...
        await client.CreateImageAsync(MediaModelCatalog.Gen4ImageTurbo, "safe prompt", "720:1280");
    ...
    public async Task Sekil_DuzgunMarsrutaVeSaheləreGedir()
    {
        ...
        await client.CreateImageAsync(MediaModelCatalog.Gen4ImageTurbo, "a warm canyon", "720:1280");
        ...
        Assert.Contains("\"model\":\"gen4_image_turbo\"", body, StringComparison.Ordinal);
    ...
    {
        var handler = new CapturingHandler(_ => Json("""{"id":"task-3","status":"RUNNING"}"""));
        var client = NewClient(handler);
        ...
        Assert.Equal(RunwayTaskState.Running, task.State);
    ...
    public async Task HttpXetasi_SebebKimiKodQaytarir()
    {
        ...
        var task = await client.CreateImageAsync(MediaModelCatalog.Gen4ImageTurbo, "p", "720:1280");
